...
Build a bilibili-video-reading release artifact:
  bvr-<version>.tar.gz
...
init = (root / "src" / "bilibili_video_reading" / "__init__.py").read_text(encoding="utf-8")
...
ARTIFACT="bvr-$VERSION.tar.gz"
ARTIFACT_PATH="$OUT_DIR/$ARTIFACT"
CHECKSUM_PATH="$ARTIFACT_PATH.sha256"
MANIFEST_PATH="$OUT_DIR/manifest.json"
INSTALLER_SH="$OUT_DIR/install_remote.sh"

TMP_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/bvr-release.XXXXXX")
cleanup() {
  rm -rf "$TMP_PARENT"
}
trap cleanup EXIT INT TERM

RELEASE_ROOT="$TMP_PARENT/bvr-$VERSION"
...
if [ -d "$RELEASE_ROOT/docs/_generated" ]; then
  rm -rf "$RELEASE_ROOT/docs/_generated"
  mkdir -p "$RELEASE_ROOT/docs/_generated/docdev" "$RELEASE_ROOT/docs/_generated/skillcli"
fi

rm -f "$ARTIFACT_PATH" "$CHECKSUM_PATH" "$MANIFEST_PATH" "$INSTALLER_SH"
tar -czf "$ARTIFACT_PATH" -C "$TMP_PARENT" "bvr-$VERSION"
...
printf '[bvr package] artifact: %s\n' "$ARTIFACT_PATH"
printf '[bvr package] checksum: %s\n' "$CHECKSUM_PATH"
printf '[bvr package] manifest: %s\n' "$MANIFEST_PATH"
printf '[bvr package] installer: %s\n' "$INSTALLER_SH"
