...
DEFAULT_REPO=${BVR_RELEASE_REPO:-hongzhiyin/bilibili-video-reading}
VERSION=${BVR_VERSION:-latest}
RELEASE_BASE_URL=${BVR_RELEASE_BASE_URL:-}
INSTALL_ROOT=${BVR_INSTALL_ROOT:-"$HOME/.local/share/bvr"}
BIN_DIR=${BVR_BIN_DIR:-"$HOME/.local/bin"}
LOG_PREFIX=${BVR_INSTALL_LOG_PREFIX:-"[bvr install]"}
SYNC_SKILL=1
...
Options:
  ...
  --install-root DIR         Install root. Default: ~/.local/share/bvr
  --bin-dir DIR              Launcher directory. Default: ~/.local/bin
  --sync-skill               Sync installed skill targets after install. Default.
  --no-sync-skill            Skip installed skill sync after install

Environment:
  BVR_RELEASE_REPO           GitHub repo, owner/name. Default: hongzhiyin/bilibili-video-reading
  BVR_RELEASE_BASE_URL       Override asset base URL
  BVR_INSTALL_ROOT           Override install root
  BVR_BIN_DIR                Override launcher directory
...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/bvr-install.XXXXXX")
cleanup() {
  rm -rf "$TMP_DIR"
}
...
if [ ! -d "$TMP_RELEASE/src/bilibili_video_reading" ] || [ ! -f "$TMP_RELEASE/skill/SKILL.md" ]; then
  printf 'install_remote: artifact does not look like a bilibili-video-reading release\n' >&2
  exit 1
fi
...
LAUNCHER="$BIN_DIR/bvr"
cat > "$LAUNCHER" <<EOF
#!/bin/sh
BVR_PROJECT_DIR="$CURRENT" PYTHONPATH="$CURRENT/src\${PYTHONPATH:+:\$PYTHONPATH}" exec python3 -m bilibili_video_reading.cli "\$@"
EOF
chmod +x "$LAUNCHER"
...
if [ "$SYNC_SKILL" -eq 1 ]; then
  sh "$TARGET_DIR/scripts/sync_skill.sh" --targets codex,cursor,agents,claude --force
  sh "$LAUNCHER" tools doctor
else
  sh "$LAUNCHER" tools check >/dev/null
fi
